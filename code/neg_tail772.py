...
N=(2*PI).sqrt()*y+3
a8=(PI/4)*arb('0.772').sqrt(); x0=a8*yl
kap=(2*PI*x0).sqrt()*(-x0).exp()*x0.bessel_i(1)
Uenv=lambda x: x.exp()/(2*PI*x).sqrt()
Main=(PI/2)*(1-(PI*arb('0.228')/4)**2/6)*(PI/4)*arb('0.772').sqrt()/y*kap*Uenv(a8*yl)      # lower bound, degenerate case
Main2=2*arb('0.5698')*(PI/4)*arb('0.772').sqrt()/y*kap*Uenv(a8*yl)           # nondegenerate
Pup=lambda k: (2*PI/k)/yl*Uenv((2*PI/k)*y)
a24=2*PI/24
D24=arb('0.506')+1/(2*yl*yl)+arb('0.5026')*a24*y*(1+1/(a24*arb('0.772').sqrt()*yl))
Sg=arb('0.5')+N/8
k16=(PI/2)*Pup(16)
k24=(N/4)*(N/2)*(N*N/2+arb('0.01')+D24)*Pup(24)
arcs=Sg*2*E1*PI.exp()*((N*N/2+1)*(PI/R2)/1+(2+PI/2)/arb('0.772')+1)
