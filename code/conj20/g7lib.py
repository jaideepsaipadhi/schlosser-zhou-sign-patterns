...
def shifts(k):          # list of (h, s(h,k)-s(h,k/7), h') for 7|k,  hh' = 1 mod k
    if k not in SH:
        SH[k]=[(h,dedekind(h,k)-dedekind(h,k//7),pow(h,-1,k)) for h in range(k) if math.gcd(h,k)==1]
    return SH[k]
...
K1=arb(7).sqrt()*(-PI/14).exp()*f_prod((-2*PI/7).exp())*f_prod((-2*PI).exp())*(1+arb(10)**-40)
def F7m(d,extract):     # f(t)^d f(t^7)^d - 1 - [extract] d t  at t=e^{-2pi}
    t=(-2*PI).exp()
    return f_prod(t)**d*f_prod(t**7)**d-1-(d*t if extract else 0)
def Econ(d,N):          # error constant, d = upper end of delta-interval (E increasing in d)
    ext = (d>4)
    t=(-2*PI).exp()
    S=arb(6)/49
    a=R2*PI*S*E1*(PI*d/2).exp()*(1+(d*t if ext else 0))
    b=2*R2*S*E1*(PI*d/2).exp()*F7m(d,ext)
    c=2*R2*E1*K1**d
    return a+b+c
